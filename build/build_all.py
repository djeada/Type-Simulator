...
def collect_artifacts(dist_dir: Path, bin_name: str, platform_key: str) -> list[Path]:
    ...
    if platform_key in ("linux", "windows"):
        ...
        # Rename the bundle, then zip & clean up
        bundle_target = dist_dir / f"{bin_name}{suffix}.app"
        app_bundle.rename(bundle_target)

        zip_path = dist_dir / f"{bin_name}{suffix}.zip"
        shutil.make_archive(
            str(zip_path.with_suffix('')), 'zip',
            root_dir=dist_dir, base_dir=bundle_target.name
        )
        logging.info("Created macOS zip archive: %s", zip_path)
