...
def test_input_validator_cases(case):
    ...
    assert is_valid == case["is_valid"], f"Case '{case['name']}' validity mismatch. Errors: {errors}, Warnings: {warnings}"
    for err_sub in case.get("errors", []):
        assert any(err_sub in e for e in errors), f"Case '{case['name']}' missing error: {err_sub}"
    for warn_sub in case.get("warnings", []):
        assert any(warn_sub in w for w in warnings), f"Case '{case['name']}' missing warning: {warn_sub}"
